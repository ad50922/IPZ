...
with (mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.5) as face_detection,
    ...
    while not quit_condition:
        ...
        y_offset = int(frame_height / (zoom * 2.0)) # tutaj wartości można zmieniać
        x_offset = int(frame_width / (zoom * 2.0))
        if results.detections:
            for detections in results.detections:
                ...
                if previous_center is None:
                    break
                elif euclideanDistance(current_center, previous_center) < 150:
        ...
        if results.multi_face_landmarks:
            ...
            right_mean_coords = np.asarray([np.mean([mesh_coords[p] for p in [4, 94]], axis=0),
        ...
        frame = utils.textWithBackground(frame, f'shape: {frame.shape}', FONTS, 0.75, (350, 50), bgOpacity=0.9,
                                         textThickness=2)
        frame = utils.textWithBackground(frame, f'zoom: {zoom}', FONTS, 0.75, (700, 50), bgOpacity=0.9,
                                         textThickness=2)
